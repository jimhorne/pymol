...
def plot_histogram_sizes(input_data, key):
    ...
    # Plot a histogram
    plt.hist(values, bins=100, color='blue', edgecolor='black') # choose num bins or statistical rule forbinsize (auto, scott, sturges, fd, doane, rice, sqrt)
...
def trim_and_align_pymol(reference, sample, ctrim, ntrim, counter='1', align='align'):
    ...
    parts = sample.split('-')
    sample_name = f'sample_{parts[1]}_trim_aligned.pdb'
    cmd.save(sample_name, 'sample')
    if counter < 2:
        cmd.save('reference_trim_aligned.pdb', 'reference')
...
globPattern = r'PAP_pdbs\40pc_base\AF*.pdb'
...
reference_pdb = r'reference_oriented_AF-P0AE06_acrAec_topump.pdb'
...
precalc_output_file = r'C:\Users\james\OneDrive\Cambridge Postdoc Project Docs\Projects\OMF-PAP_length_distributions\interpro_35pc_resclust_tolc\output_interproTolC35pc_aligned_to_tolC_withSuper.tsv'
